...
weight_diff(WeightList) -> 
    Weights = lists:map(fun({_, W}) -> W end, WeightList),
    lists:max(Weights) - lists:min(Weights).

is_balanced(WeightList) -> lists:all(fun({_, W}) -> W == element(2, hd(WeightList)) end, WeightList).

solve(Name, LastWeight, Diff) -> 
    case select_faulty_tower(node_weights(Name)) of
        {{X, W}, 0} -> LastWeight - Diff;
        {{X, W}, Diff} -> solve(X, element(1, maps:get(X, input_map())), Diff)
    end.
